...
export default function PlayersPage() {
  ...
  const getUpdatedPlayers = () => {
    let updatedPlayersUP = data?.getPlayers?.data;
    if (teamId && teamId !== 'Select a team' && teamId?.length > 0) {
      updatedPlayersUP = data?.getPlayers?.data?.filter(
    ...
    updatedPlayersUP = _.orderBy(updatedPlayersUP, (item: any) => item.player.rank, ['asc']);
    setUpdatedPlayers(updatedPlayersUP);
  };
  ...
  const filteredData = () => {
    let updatedPlayersFF = data?.getPlayers?.data;
    if (teamId && teamId !== 'Select a team' && teamId?.length > 0) {
      updatedPlayersFF = data?.getPlayers?.data?.filter(
        (current: { player: { teamId: string; leagueId: string } }) =>
          current?.player?.teamId === teamId && (leagueId === 'Select a league' || leagueId?.length === 0 ? true : current?.player?.leagueId === leagueId),
      );
    } else if (leagueId && leagueId !== 'Select a league' && leagueId?.length > 0) {
  ...
  const updateRank = async (newPlayers: string | any[]) => {
    ...
    const rankUpdatePlayerMutationList = [];
    for (let i = 0; i < newPlayers?.length; i += 1) {
      updateNeeded = true;
      setShowLoader(true);
      rankUpdatePlayerMutationList.push(
        rankUpdatePlayerMutation({
          variables: {
    ...
    await Promise.all(rankUpdatePlayerMutationList);
    if (updateNeeded) {
                      ...
                      (player: any, prevIndex: number) =>
                        player?.mapPlayer?.map((current: any, index: number) => (
                          <React.Fragment key={player._id}>
                            {index === 0 && (
                              <Draggable draggableId={player._id} index={prevIndex} isDragDisabled={selectedIds.includes(teamId)}>
                                {(provided, snapshot) => (
                                  <tr
                                  ...
                                  >
                                    <TD>
                                      {player?.mapPlayer?.length > 1 ? (
                                        <button className="text-white px-4 py-2 rounded" type="button" onClick={() => setIsOpen(isOpen?.length > 0 ? '' : player._id)}>
                                          {isOpen === player._id ? (
