...
import { ADD_UPDATE_COACH } from '@/graphql/coach';
...
export default function AddUpdateCoach(props: AddUpdateCoachProps) {
  ...
  const [active, setActive] = useState(props?.coach ? `${props?.coach?.active}` : 'true');

  const [addUpdateCoach, { data, error, loading }] = useMutation(ADD_UPDATE_COACH, {
    variables: {
      firstName,
      lastName,
      email,
        ...
        <form className="form w-100">
          <div className="flex flex-row flex-wrap">
            ...
            <div className="w-full md:w-1/2 lg:w-1/3 my-2">
              ...
              />
              <label htmlFor="default-checkbox" className="ml-2 font-bold text-black ">
                Mark as a Player
              </label>
            </div>
          </div>
          ...
          <div className="my-2">
            ...
            <button
              ...
              className="transform hover:bg-red-600 transition duration-300 hover:scale-105 text-white bg-red-500 font-medium rounded-lg text-sm px-6 py-3.5 text-center inline-flex items-center mr-2 mb-22"
            >
              Cancel
            </button>
          </div>
